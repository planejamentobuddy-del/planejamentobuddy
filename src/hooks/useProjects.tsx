...
// Inicialização dinâmica do Supabase do app de Estoque (100% de graça!)
const ESTOQUE_URL = import.meta.env.VITE_SUPABASE_ESTOQUE_URL || '';
const ESTOQUE_KEY = import.meta.env.VITE_SUPABASE_ESTOQUE_ANON_KEY || '';
const supabaseEstoque = (ESTOQUE_URL && ESTOQUE_KEY) ? createClient(ESTOQUE_URL, ESTOQUE_KEY) : null;
    ...
    try {
      ...
      if (planData) {
        setPlans(planData.map(p => {
          ...
          return {
            ...
            subtasks: p.subtasks || '',
            expectedProgress: p.expected_progress !== undefined ? Number(p.expected_progress) : 100,
            currentProgress: p.current_progress !== undefined ? Number(p.current_progress) : 0,
          };
        }));
      }
      ...
      if (supplyData) {
        setSupplyPackages(supplyData.map((s: any) => ({
          ...
          quantitative: s.quantitative || undefined,
          arriveBy: s.arrive_by || undefined,
          daysBeforeOrder: s.days_before_order || undefined,
  ...
  const addWeeklyPlan = useCallback(async (plan: Omit<WeeklyPlan, 'id'>) => {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        expected_progress: plan.expectedProgress !== undefined ? plan.expectedProgress : 100,
        current_progress: plan.currentProgress !== undefined ? plan.currentProgress : 0,
      }])
      .select()
      .single();
    ...
    if (!error && data) {
      setPlans(prev => [...prev, {
        ...
        expectedProgress: data.expected_progress !== undefined ? Number(data.expected_progress) : 100,
        currentProgress: data.current_progress !== undefined ? Number(data.current_progress) : 0,
      }]);
    } else if (error) {
  ...
  const addSupplyPackage = useCallback(async (pkg: Omit<SupplyPackage, 'id' | 'createdAt'>): Promise<SupplyPackage | null> => {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        quantitative: pkg.quantitative || null,
        arrive_by: pkg.arriveBy || null,
        days_before_order: pkg.daysBeforeOrder || null,
        responsible: pkg.responsible || null,
    ...
    const newPkg: SupplyPackage = {
      ...
      quantitative: data.quantitative || undefined,
      arriveBy: data.arrive_by || undefined,
      daysBeforeOrder: data.days_before_order || undefined,
